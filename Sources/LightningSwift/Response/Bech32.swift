...
public class Bech32 {
    ...
    public func decode(_ str: String, length: Int = 90) throws -> (hrp: String, checksum: Data) {
        let (hrp, checksum, _) = try decodeM(str, length: length)
        return (hrp,checksum)
    }
    
    public func decodeM(_ str: String, length: Int) throws -> (hrp: String, checksum: Data, encoding: Encoding) {
        guard let strBytes = str.data(using: .utf8) else {
            throw DecodingError.nonUTF8String
        }
        guard strBytes.count <= length else {
